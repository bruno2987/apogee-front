<template>  <!-- code très proche de articles et workshop mais qui requête dans la collection evenements -->
    <div>
        <button id="boutonOrange" v-if="unEvenement" @click="retourListe">Retour</button>
        <div id="listeEvenement" v-if="!unEvenement"> 
        ...
        <Evenement v-if="unEvenement" :evenementSent="unEvenement"/>
...
export default {
    ...
    methods: {
        getOnEvenement(id){
            this.http.get('http://localhost:3000/users/getOneEvenement/'+id) 
                .then((response) => {
                    this.unEvenement = response.data.article
                })
                .catch((erreur)=>
                    console.log(erreur)
    ...
    beforeMount(){
        if(this.sideEvnmt){
            this.unEvenement = this.sideEvnmt
        }
    }
}
</script>
