<template>
    <div id="nouveauContenu">
        <div class="radioTypecontenu">
            <div id="artRad">
                <div class="pRadio">
                    <input id="radioArticle" type="radio" for="article" name="typeContenu" value="article" v-model="typeContenu">
                    <label for="article">Article</label>
                </div>
            </div>
            <div id="evenRad">
                <div class="pRadio">
                    <input id="radioEven" type="radio" for="evenement" name="typeContenu" value="evenement" v-model="typeContenu">
                    <label for="evenement">Evènement</label>
                </div>
            </div>
            <div id="workRad">
                <div class="pRadio">
                    <input id="radioWork" type="radio" for="workshop" name="typeContenu" value="workshop" v-model="typeContenu">
...
export default {
    ...
    methods: {
        record: function(){
            switch(this.typeContenu){
                case 'article':
                    this.http.post('http://localhost:3000/admin/articles/updateArticle',{
                        id:this.objectSent._id,
                        ...
                        imagesAncienneVersion: this.objectSent.imgArticle  // On envoie les photos qu'il y avait avant modification car le server fera la comparaison entre cette liste d'image et celles qui sont effectivement dans le corps du text pour supprimer celles qui ne servent pas.
                        },
                        {headers: 
                ...
                case 'evenement':
                    this.http.post('http://localhost:3000/admin/articles/updateArticle',{
                        ...
                        dateEvnmt: this.dateEvnmt,
                        heureEvnmt: this.heureEvnmt,
                        imagesAncienneVersion: this.objectSent.imgArticle,
                        nVoie: this.nVoie,
    ...
    beforeMount() {
        this.typeContenu = this.contenuSent
        this.titre = this.objectSent.titre
        this.article = this.objectSent.corps
        this.resume = this.objectSent.resume
        this.objectSent.nbrPlace? this.nbrPlace = this.objectSent.nbrPlace : this.nbrPlace = ''
        this.objectSent.dateEvnmt? this.dateEvnmt = this.objectSent.dateEvnmt : this.dateEvnmt = ''
        this.objectSent.heureEvnmt? this.heureEvnmt = this.objectSent.heureEvnmt : this.heureEvnmt = ''
        if(this.objectSent.adresse){
            this.commune=this.objectSent.adresse.commune
            this.zip=this.objectSent.adresse.zip
            this.nVoie = this.objectSent.adresse.nVoie
            this.objectSent.adresse.description? this.description = this.objectSent.adresse.description :  this.description = ''
        }
    },
    mounted(){
        switch(this.contenuSent){
            case 'article':
                document.getElementById('radioEven').setAttribute("disabled", "")
                document.getElementById('radioWork').setAttribute("disabled", "")
                break;
            case 'evenement':
                document.getElementById('radioArticle').setAttribute("disabled", "")
                document.getElementById('radioWork').setAttribute("disabled", "")
                break;
            case 'workshop':
                document.getElementById('radioArticle').setAttribute("disabled", "")
