<template>
    <div id="article">
        <button class="boutonOrange" v-if="unArticle" @click="retourListe">Retour</button>
        <div id="listeArticle" v-if="!unArticle"> <!-- La liste ne s'affiche que si l'attribut article dans le store (vuex) est vide. -->
        ...
        <Article v-if="unArticle" :articleSent="unArticle"/> <!-- Ne s'affiche que si l'attribut article du store contient un article -->
...
export default {
    ...
    methods: {
        getOneArticle(id){
            this.http.get('http://localhost:3000/users/getOneArticle/'+id)  // Cette fonction permet de chercher l'article correspondant à l'id et de le mettre dans le store
                .then((response) => {
                    this.unArticle = response.data.article
                })
                .catch((erreur)=>
    ...
    beforeMount(){
        if(this.sideArticle){
            this.unArticle = this.sideArticle
        }
    }
}
</script>
